'''
This file implements the data loader for LidarStereoNet's test/eval phase.
...
For test/eval, the ground truth images are also required.
'''
...
class KittiLoader(data.Dataset):
    def __init__(self, left, right, lidar_left, lidar_right, training, ground_truth=None, loader=image_loader, transform=None):
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            ground_truth_img = self.loader(ground_truth)

            ####
            #h, w = left_img.shape[0:2]
            #th, tw = 256, 512

            #y1 = random.randint(0, w - tw - 100)
            #x1 = random.randint(0, h - th - 100)

            #left_img = left_img[x1:x1+th, y1:y1+tw, :]
            #right_img = right_img[x1:x1+th, y1:y1+tw, :]
            #lidar_left_img = lidar_left_img[x1:x1+th, y1:y1+tw]
            #lidar_right_img = lidar_right_img[x1:x1+th, y1:y1+tw]
            #ground_truth_img = ground_truth_img[x1:x1+th, y1:y1+tw]
            ###
            ...
            ground_truth_img = ground_truth_img[h-368:h, w-1232:w]
            orig_left_img = np.copy(left_img).astype(np.uint8)
        ...
        ground_truth_img = torch.tensor(ground_truth_img).float().unsqueeze(dim=0) / 256

        return left_img, right_img, lidar_left_img, lidar_right_img, ground_truth_img, orig_left_img
